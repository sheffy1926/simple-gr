...
double cpuTime(void)
{
    ...
    return static_cast<double>(cputime.ru_utime.tv_sec) + (1.e-6) * static_cast<double>(cputime.ru_utime.tv_usec)
           + static_cast<double>(cputime.ru_stime.tv_sec) + (1.e-6) * static_cast<double>(cputime.ru_stime.tv_usec);
}
...
void SimpleGR::printStatistics(bool checkRouted, bool final)
{
    ...
    cout << stringFinal << "total wire length "
         << static_cast<CostType>(routedLen) + viaFactor * static_cast<CostType>(numVias) << endl;
...
void SimpleGR::printStatisticsLight(void)
{
    ...
    if (numLayers > 1) {
        ...
        cout << "total wire length "
             << static_cast<CostType>(totalSegments) + viaFactor * static_cast<CostType>(totalVias) << endl;
    }
...
void SimpleGR::buildGrid(void)
{
    ...
    for (unsigned k = 0; k < numLayers; ++k) {
        ...
        for (unsigned j = 0; j < gcellArrSzY; ++j) { gcellArr3D[k][j].resize(gcellArrSzX); }
    }
...
bool PQueue::isGCellVsted(IdType gcellId) const { return dataValid.isBitSet(gcellId); }
